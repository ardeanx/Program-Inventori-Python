# import modul yang dibutuhkan untuk program python ini
from tkinter import *
...
# inisiasi awal untuk pembuatan GUI (Disini saya pake model Treeview)
window=tkinter.Tk()
window.title("Program Inventori")
window.geometry("720x640")
my_tree=ttk.Treeview(window,show='headings',height=20)
style=ttk.Style()

# Mengganti Icon default Tkinter dengan Icon Pilihan anda. Kalau ada kesalahan berarti PATH tidak icon tidak benar.
window.iconbitmap("C:/Users/ardea/Documents/Python/inventori-main/logo.ico")

# Inisialisasi Array Kosong, String Karakter Numerik, dan Karakter Huruf Besar.
placeholderArray=['','','','','']
numeric='1234567890'
alpha='ABCDEFGHIJKLMNOPQRSTUVWXYZ'

# Buat Koneksi ke Database. Pada Program ini saya menggunakan koneksi Mysql dengan mysql-connector
def connection():
...
# Mengkoneksikan ke DB dan mendapatkan objek kursor yang dapat menjalankan kueri dan mengambil hasil.
conn=connection()
cursor=conn.cursor()

# Inisialisasi array kosong dengan 5 elemen untuk menyimpan objek tkinter StringVar
for i in range(0,5):
...
# Menetapkan nilai elemen pada nomor indeks di Array placeholder ke kata yang diberikan.
def setph(word,num):
    for ph in range(0,5):
        if ph == num:
...
def save():
    ...
    if not(itemId and itemId.strip()) or not(name and name.strip()) or not(price and price.strip()) or not(qnt and qnt.strip()) or not(cat and cat.strip()):
        messagebox.showwarning("","Silahkan isi semua entri")
        return
    if len(itemId) < 5:
        messagebox.showwarning("","ID Item tidak benar, silahkan klik Generate")
        return
    if(not(itemId[3]=='-')):
        valid=False
    for i in range(0,3):
    ...
    if(not(itemId[4] in alpha)):
        valid=False
    if not(valid):
        messagebox.showwarning("","ID Item tidak benar")
        return
    try:
        cursor.connection.ping()
        ...
        print(e)
        messagebox.showwarning("","Kesalahan ketika menyimpan ref: "+str(e))
        return
    refreshTable()
...
def update():
    ...
    try:
        ...
        messagebox.showwarning("", "Silahkan pilih baris data")
    ...
    if(selectedItemId!=itemId):
        messagebox.showwarning("","Anda tidak dapat mengganti ID Item")
        return
    try:
        cursor.connection.ping()
        ...
        messagebox.showwarning("","Terdapat Kesalahan ref: "+str(err))
        return
    refreshTable()
        ...
        if(my_tree.selection()[0]):
            ...
            if(decision != 'yes'):
                ...
                try:
                    ...
                    messagebox.showinfo("","Maaf, terdapat kesalahan")
                refreshTable()
    except:
        messagebox.showwarning("", "Silahkan pilih baris data")

# Fungsi ketika pilih baris data
def select():
    try:
        selectedItem = my_tree.selection()[0]
        itemId = str(my_tree.item(selectedItem)['values'][0])
...
# Fungsi Membersihkan entri termasuk pada Array
def clear():
    for num in range(0,5):
        setph('',(num))
...
manageFrame=tkinter.LabelFrame(frame,text="Kelola",borderwidth=5)
...
updateBtn=Button(manageFrame,text="PERBARUI",width=10,borderwidth=3,bg=btnColor,fg='white',command=update)
...
findBtn=Button(manageFrame,text="CARI",width=10,borderwidth=3,bg=btnColor,fg='white',command=find)
clearBtn=Button(manageFrame,text="BERSIHKAN",width=10,borderwidth=3,bg=btnColor,fg='white',command=clear)
exportBtn=Button(manageFrame,text="EXPORT KE EXCEL",width=15,borderwidth=3,bg=btnColor,fg='white',command=exportExcel)
...
qntLabel=Label(entriesFrame,text="JUMLAH",anchor="e",width=10)
...
# Tambahkan Kategori disini
categoryArray=['Makanan','Minuman','Rokok','Mainan','Pakaian', 'Elektronik','Obat','Herbal','Sembako','Lainnya']
...
generateIdBtn=Button(entriesFrame,text="Buat ID",borderwidth=3,bg=btnColor,fg='white',command=generateRand)
...
my_tree.heading("Item Id",text="ID Item",anchor=W)
my_tree.heading("Name",text="Nama",anchor=W)
my_tree.heading("Price",text="Harga",anchor=W)
my_tree.heading("Quantity",text="QTY",anchor=W)
my_tree.heading("Category",text="Kategori",anchor=W)
my_tree.heading("Date",text="Tanggal",anchor=W)
my_tree.tag_configure('orow',background="#EEEEEE")
my_tree.pack()
